...
export default function DisplayAllRestaurant() {
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      ...
      var body={"restaurantname": restaurantName,"ownername": ownerName,"phonenumber": phoneNumber,"emailid":emailid,"mobileno": mobileNumber,"address": address,"stateid": stateid,"cityid": cityid,"url": url,"fssai": fssai,"gstno": gstNo,"gsttype":gstType,"updatedat": cd, restaurantid:restaurantid}
  ...
  else if(imgStatus==2)
  {
    ...
    setFileFssai({url:tempFile.shopAct,bytes:''})
  }
  else if (imgStatus==3)
  {
    setBtnStatus((prev)=>({...prev,logo:false}))
    setFileFssai({url:tempFile.logo,bytes:''})
  }
}
